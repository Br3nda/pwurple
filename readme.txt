...
V3.0 
- Project forked. pwurple_php has released final version. Project renamed to pwurple.
- Goals are:  1) php5-ification. 2) Small memory footprint.


V2.1
- Added patch parameter for ease of initial configuration in pwurple_config.php
...
V2.1 Beta3
- Patch debugging feature. Added to the pwurple_config a new constant called
...
- pwurple_class.php: removed the check for strings such as MSIE or Windows in the
...
- pwurple_config.php: added the constant for the pwurple_patch commented.
	Easier to add reference to a patch.
- no multipatch support, yet.

V2.1 Beta2
- new search algorithm for unknown user agents, much faster
- added pwurple_config.php for cleaner configuration parameters
...
WARNING
=============
pwurple_class.php used to be incompatible with the web patch.
...
WURFL_PATCH_FILE  string, optional patch file for WURFL
WURFL_AGENT2ID_FILE   string, used by pwurple_class.php. Is refreshed when a new
                      WURFL is found
WURFL_LOG_FILE    string, defines full path and filename for logging
WURFL_AUTOLOAD    boolean, true if you want the XML to be loaded at every
                  startup. If not, the XML will be loaded when needed.
...
The class is initialized calling the constructor, pwurple_class and passing two
...
$pwurple_class->_wurfl          is the WURFL array (all of it)
$pwurple_class->_pwurple_agents   is the associative array made of the user agents
                              and unique id's
$pwurple_class->user_agent      the visitor's user agent
$pwurple_class->pwurple_agent     the WURFL's best fitting user agent
$pwurple_class->id              the corresponding id
$pwurple_class->GUI             true if the device supports Openwave's GUI
                              extensions
$pwurple_class->is_wireless_device  true if the device is WAP capable. To take
...
$pwurple_class->capabilities    the array of device's capabilities

Note: PHP (up to version 4.3) does not have any distinction between private and
public methods. In the pwurple_class implementation, I named all private methods
...
pwurple_class($wurfl, $pwurple_agents)   is the constructor. Built to work
                                     best with the pwurple_parser.
...
Extra features in pwurple_class: after some tests we noticed that the biggest
...
6) Extract example code from bottom of readme.txt (this file), save to pwurple_test.php in convenient place on your server
7) Send WWW query to http://<server>/pwurple_test.php ---> Get message "Only offering WAP service"
8) Send WAP query [using phone] to http://<server>/pwurple_test.php ---> See "Home" link, (and outline for missing image)
9) Enjoy!
...
$myDevice = new pwurple_class($wurfl, $pwurple_agents);
